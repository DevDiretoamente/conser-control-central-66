  ...
  return (
    <div className="flex justify-end mb-4">
      <div className="inline-flex rounded-lg border p-1 shadow-sm">
        <button
          className={`px-4 py-2.5 rounded-md text-sm font-medium transition-colors ${visualizacao === 'individual' ? 'bg-primary text-white' : 'hover:bg-muted'}`}
          onClick={() => setVisualizacao('individual')}
        >
          Individual
        </button>
        <button
          className={`px-4 py-2.5 rounded-md text-sm font-medium transition-colors ${visualizacao === 'calendario' ? 'bg-primary text-white' : 'hover:bg-muted'}`}
